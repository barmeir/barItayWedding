// `answers` is matched case-insensitively after trim. Add as many variants as you want.
// For multiple-choice stages: set `options` (array of strings) and `correctAnswer` (string).
// `accent` is one of: cyan, magenta, violet, gold (used for the stage tint).

export const stages = [
  
  {
    id: 0,
    type: 'story',
    title: 'עזרו לנו למצוא את המטמון 🪎',
    slides: [
 'הוחבאו...\nהטבעות שלנו!!!',
      'עד טקס החתונה נצטרך את הטבעות.\nמי יצילם?',

 'נסו להשתמש בראש...\nביצירתיות.',

      'בכל הרמזים כאן למטה.\n< לא להעתיק 😏>\nנסו לבד לפני שתעזרו באדם אחר, זה ממש כיף!!',
      'המשחק הוא בהמשכים. \nאם תצאו מהאתר,\nזה שומר את השלבים ✅ ותמשיכו גם מחר',
      'אם נתקעתם, הכל בסדר\nשאלו אותנו.\n 🤍',

    ],
    caption: 'משחק חידות בהמשכים 💌',
    accent: 'magenta',
  }, 

  {
    id: 1,
    title: 'היעד הסודי',
    
    riddle: `ביומיים הקרובים נבלה בריזורט בשם
    _________    ______ .
    
    _________ <- אי אגדי המוזכר בכתביו של הפילוסוף היווני שאיתי מחבב

    ______ <- נמצא מחוץ לכל ארמון מפואר`,
    answers: ['אטלנטיס גרדנס', 'אטלנטיק גרדן', 'אטלנטיס גרדנס', 'אטלנטיס גרדן', 'atlantis', 'atlantis garden', 'atlantis gardens'],
    hints: [
      'אפלטון כתב עליה קודם + מקום בוטני.',
      ' יש עיר מיתולוגית ששקעה בים + יש שם הרבה פרחים.',
      'אטלנ_ _ _ "Garden".',
    ],
    image: 'images/stage-01.jpg',
    caption: 'גן העיר האבודה 🌊 🌴 סעו לשם',
    accent: 'violet',
  },

  {
    id: 2,
    title: ' ההיכרות',
    riddle: `לפני שהיו משפחה,
    לפני הטיולים,
    לפני ההתלבטויות על מסעדה -
    היו רק שני אנשים..
    
    איך איתי ובר הכירו?`,
    options: [
      'בחברת הייטק',
      'בקבוצת פייסבוק',
      'בפיקניק טבעוני קהילתי',
      'דרך חברים של חברים',
      '   ',
      ],
    correctAnswer: 'בקבוצת פייסבוק',
    hints: [
      'הם נפגשו בעבודה?',
      'טבעוני טעים.',
      'בר התחילה עם איתי.',
    ],
    image: 'images/stage-03.jpg',
    caption: 'כל סיפור טוב מתחיל בצעד קטן',
    accent: 'cyan',
  },
  {
    id: 3,
    title: 'הקוד בחדר שלכם',
    riddle:
      `יש כל מיני זוגות,
אחד מהם מגיע מוקדם
האחרת מאחרת
אז בממוצע הם מגיעים בזמן?

                (קיראו את הרמזים)`,
    answers: ['012','123', '234', '345', '456 ', '567', '678', '789', '890', '901'],
    hints: [
      'יש לכם משהו מאיתנו,',
      'חפשו את הקוד בתוך המעטפה בחדר.',
      'מספר בסדרה עולה.',
    ],
    image: 'images/stage-05.jpg',
    caption: 'אחד מכם מסתיר מידע 🤫',
    accent: 'violet',
  },
  {
    id: 4,
    title: 'גיבורים על העל',
    riddle:
     '  הגיבור האהוב של איתי הוא:',
    answers: ['betman', 'באטמן', 'בטמן','batman', 'the batman','betmen'],
    hints: [
      'יש להם לגו קיר ענק בבית .',
      'גלימה, מסכה שחורה .',
      'וקו רקע אפלולי .',
    ],
    image: 'images/stage-06.png',
    caption: 'לכבוד שומר הלילה 🦇',
    accent: 'violet',
  },
   {
    id: 5,
    title: 'בוקר טוב',
    riddle:
     `יש יצורים שאיתי אוהב. הוא הציל אותם.
מחבק אותם בין ידיו ומלווה אותם עד לביתם החדש במחסה בטוח.

בר, לעומת זאת… בכל יום מוצאת אותם טורדניים ומחפשת להם טיסה בכיוון אחד .`,
    answers: ['תרנגולים', 'תרנגולות', 'chickens', 'chicken', 'תרנגול', 'תרנגולת'],
    hints: [
      'יש אנשים שמתעוררים מהשעון, בר מתעוררת מטראומה יומית עם נוצות .',
      'אפשר למצוא אותי בחווה או... פשוט ביפו .',
      'אני לא יונה .',
    ],
    image: 'images/stage-11.jpg',
    caption: '🐔 באים איתכם לקליפורניה🐔',
    accent: 'magenta',
  },
  {
    id: 6,
    title: 'בריכות או ברכות?',
    riddle:
      'לכו לבריכה וחפשו שם את הרמז הבא',
    answers: ['a5', 'A5', '5a', '5A'],
    hints: [
      'המקום הזה הוא הלובי שלנו',
      'כל הקומה התחתונה היא טיילת משפחתית ',
      'מספר של הוילה שלנו',
    ],
    image: 'images/stage-08.jpg',
    caption: 'בואו לוילה!',
    accent: 'magenta',
  },
  {
    id: 7,
    title: 'המנעול הפיזי',
    riddle:
      `הטקס יהיה בלי הטבעות.
      כן, אנחנו לא שיגרתיים..
           אז תסיימו לפני יום שישי ב15:00

לכו ללובי שלנו (וילה קומת קרקע), מאחורי הדלת מחכה לכם הקוד->`,
    answers: ['220526'],
    hints: [
      'לכו ללובי של החופשה.',     
      'הספה שלכם .',
      'הגינה שלכם .',
    ],
    image: 'images/stage-04.png',
    caption: 'Friday Golden hour 🔓',
    accent: 'gold',
  },
//    caption: `כל רמז מקרב אתכם לאוצר. או לפחות לבריכה.`,
  {
    id: 8,
    title: 'קפסולת הזמן',
    riddle:
      `מה קרה בפעם הראשונה ש...
    צרפו את המספרים לפי הסדר הכרונולוגי:

      1. אני אוהב אותך ראשון
      2. טיסה ראשונה ביום הולדת
      3. מעבר דירה יחד
      4. טיול במפלים הלבנים בצפון
      5. לאכול פעם ראשונה פיצה שאיתי הכין,`,
    answers: ['54213'],
    hints: [
      'טסנו ללונדון ביום ההולדת של בר ואיתי רשם לה משוואה רומנטית שהוא אוהב אותה',
      'בדייט החמישי שלנו היינו בטיול בצפון',
      'קודם כל פיצה! אח״כ נדבר...',
    ],
    image: 'images/stage-07.jpg',
    caption: 'תמונה ששווה אלף מזל טוב 📸',
    accent: 'gold',
  },

  {
    id: 9,
    title: 'אתם מתקדמים אל האוצר💎',
    riddle:
    `* מי נרדם ראשון?
* מי תמיד רעב?
* מי אשף החידות?
* מי יכול לסוע לכל מקום ללא וויז ותגיע בוודאות?
* מי נרדם ראשון?

צרפו את האות הראשונה של כל תשובה (איתי=א׳ בר=ב׳)
צירוף האותיות יעביר אותכם שלב`,

    answers: ['בבאבא'],
    hints: [
      'כמה אתם מכירים אותנו? 🤔',
      '__אבא.',
      'נתקעתם בדרך? תאשימו את זו שמעולה בכיוונים.',
    ],
    image: 'images/stage-09.png',
    caption: 'אתם מעולים שהגעתם עד לכאן!\n     וגם כי לקח הרבה זמן לפתח את האתר 😅',
    accent: 'cyan',
  },
  {
    id: 10,
    title: 'ים המקום המושלם',
    riddle:
    'מתחת לשמשייה מחכה לכם הרמז הבא',
    answers: ['151093'],
    hints: [
      'הטבעת שם?💍',
      'המספר הוא תאריך חשוב בשבילנו.',
    'תאריך חשוב בחודש אוקטובר.'
    ],
    image: 'images/stage-10.jpg',
    caption: 'עשו סלפי אישי עם הרקע של הים ושלחו לנו!📸',
    accent: 'violet',
  },
 
  {
    id: 11,
    title: 'היו היה',
    riddle:
      `לבר ואיתי יש משהו משותף עוד מלפני שנולדו, מלפני שחשבו עליהם בכלל.
      מה השם?`,
    answers: ['מזל', 'סבתא מזל'],
    hints: [
      'לפחות 90 שנה אחורה .',
      'אין מצב שזו אותה הסבתא... .',
      'זה רק השם של הסבתא .',
    ],
    image: 'images/stage-12.png',
    caption: 'שיהיה לכולנו במזל טוב! 🍀',
    accent: 'gold',
  },

  {
    id: 12,
    title: 'ביום יום',
    riddle:
      'מה המילה הכי נפוצה שבר אומרת בכל יום?',
    answers: ['אופס', 'שיט','אופסי'],
    hints: [
      'אומרים כשמשהו נופל או משתבש.',
      'לפעמים זה מצחיק',
      '״לא נורא..״',
      'שיט',
       'או אופסי זאת המילה',
       'אופס ☺️',
    ],
    image: 'images/stage-14.jpg',
    caption: 'זה משעשע.. באמת. ✨',
    accent: 'cyan',
  },
    {
    id: 13,
    type: 'form',
    title: 'מתנה אחרונה לזוג',
    riddle:
      'הגעתם עד הסוף — כל הכבוד! 🎉\nהאוצר האמיתי הם המשפחה, החברים והאנשים שנמצאים איתכם.\n תודה שבאתם לחופשה הזו! \nבבקשה שימו פתק הצבעה- בלינק הבא: \n (אחריו לחצו על ״סיימתי״ כאן באתר)',
    formUrl:
      'https://docs.google.com/forms/d/e/1FAIpQLScf41hN50--g1WED-u3_KahyRgmCSNhZZRzQwt3DSiWqu6ilQ/viewform?usp=publish-editor',
    image: 'images/stage-13.png',
    caption: 'תודה שהיית חלק מהסיפור 💌',
    accent: 'magenta',
  },
];

export function isCorrectAnswer(stage, input) {
  if (stage.correctAnswer !== undefined) {
    return String(input || '').trim() === String(stage.correctAnswer).trim();
  }
  const cleaned = String(input || '').trim().toLowerCase();
  if (!cleaned) return false;
  return (stage.answers || []).some((a) => String(a).trim().toLowerCase() === cleaned);
}
